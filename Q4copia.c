...
/* Otimizado: Compara a distância ao quadrado para evitar o custo computacional do sqrtf */
Inimigo *encontrarInimigoMaisProximo(Inimigo *vetor, int n, Vector2 posJogador) {
    if (vetor == NULL) return NULL;

    Inimigo *maisProximo = NULL;
    float menorDistancia2 = 0.0f;
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (maisProximo == NULL || distancia2 < menorDistancia2) {
            maisProximo = ini;
...
int main(void) {
    ...
    // Validação essencial do malloc antes da inicialização
    if (inimigos == NULL) {
        CloseWindow();
        return 1;
    }
