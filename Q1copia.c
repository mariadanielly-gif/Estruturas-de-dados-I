...
/* Libera a memória da matriz de forma segura */
...
/* Cria a matriz dinâmica tratando falhas de alocação intermediárias */
int **criarMatriz(int linhas, int colunas) {
    ...
    for (int i = 0; i < linhas; i++) {
        ...
        // Se falhar a alocação de uma linha, libera as linhas anteriores e o vetor principal
        if (matriz[i] == NULL) {
...
Bola *criarBolas(int quantidade) {
    ...
    for (int i = 0; i < quantidade; i++) {
        ...
        // Garante que a velocidade não seja zero nos eixos
        float vx = (float)GetRandomValue(-4, 4);
        float vy = (float)GetRandomValue(-4, 4);
        b->vel = (Vector2){ (vx == 0) ? 2.0f : vx, (vy == 0) ? -2.0f : vy };
...
int main(void) {
    ...
    // Alocação com checagem de erros
    int **grade = criarMatriz(linhas, colunas);
    int quantidadeBolas = 12;
    Bola *bolas = criarBolas(quantidadeBolas);

    // Se qualquer alocação falhar, fecha a janela e encerra com erro
    if (grade == NULL || bolas == NULL) {
...
int main(void) {
    ...
    CloseWindow();
    return 0;
>>>>>>> d2ba91537a3a607017cdf0170007a130aac7b1a9
}